...
AI-powered marketing report assistant that turns uploaded CSV campaign data into automated performance summaries, metric dashboards, and actionable recommendations.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "287576261+anis-byte@users.noreply.github.com"
  git config user.name "anis-byte"
  ```
